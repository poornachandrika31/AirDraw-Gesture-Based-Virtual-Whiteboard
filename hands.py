...
class HandDetector():
    ...
    def __init__(self, mode = False, max_hands = 1):
        ...
        self.mode = mode
        # hand drawing stuff — use keyword args (required in mediapipe >= 0.10)
        ...
        # Initialise results so draw_landmarks() won't crash before first frame
        self.results = None
    ...
    def draw_landmarks(self, img):
        ...
        """
        # Guard: results may be None if called before the first detect_landmarks()
    ...
    def detect_gesture(self, landmarks, threshhold=0.70, debug=False):
        ...
        # All 4 fingers extended -> HOVER (open hand)
        ...
        # Index + Middle pointing out -> HOVER (Alternative hover)
        if cos_angle(palm_index_vector, index_vector) > threshhold and \
            cos_angle(palm_mid_vector, middle_vector) > threshhold and \
        ...
        # ONLY Index finger pointing out -> DRAW (Precise writing)
        if cos_angle(palm_index_vector, index_vector) > threshhold and \
            cos_angle(palm_mid_vector, middle_vector) < 0 and \
            cos_angle(palm_ring_vector, ring_vector) < 0 and \
            cos_angle(palm_pinky_vector, pinky_vector) < 0:
           return Gesture.DRAW
    ...
    def get_gesture_metadata(self, frame):
        ...
        # Use RAW landmark_list for tracking so the pen feels instant and has no dragging delay
        ...
        # Apply a 1.4x Trackpad-style Multiplier outwards from center so users can easily reach all UI edges!
        h, w = frame.shape[:2]
        def scale(val, max_val): return int((val - max_val/2) * 1.4 + max_val/2)
            
        index_r, index_c = scale(index_r, h), scale(index_c, w)
        mid_r, mid_c = scale(mid_r, h), scale(mid_c, w)
        ring_r, ring_c = scale(ring_r, h), scale(ring_c, w)
        pinky_r, pinky_c = scale(pinky_r, h), scale(pinky_c, w)
        ...
        if gesture == Gesture.DRAW:
            # We now use ONLY the index finger for extreme precision
            index_r, index_c = index_fing_tip
            post['origin'] = (int(index_r), int(index_c))
            post['radius'] = 6.0 # fixed small radius for the cursor
